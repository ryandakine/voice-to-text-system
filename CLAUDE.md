...
- cimco_assistant.py and cimco_tools.py are legacy files — not part of the active pipeline.

## Memory (MemPalace)

This project is wing `voice-to-text-system` in MemPalace. Before answering questions
about this project's history, decisions, or past bugs, query the palace:

1. **On wake-up:** Call `mempalace_status` to load palace overview + protocol.
2. **Before claiming a fact:** Call `mempalace_search` (mode=hybrid) or
   `mempalace_kg_query` first. Wrong is worse than slow.
3. **After making a decision:** Call `mempalace_kg_add` to record it.
   Use exclusive predicates (deployed_to, current_branch, uses_db) so
   conflicts auto-resolve.
4. **End of session:** Call `mempalace_diary_write` with what shipped,
   what broke, what's next.

Search this project's wing only:
`mempalace_search(query="...", wing="voice-to-text-system")`

## Project Context
Query `osi-context.get_bundle("voice-to-text-system")` at session start for project decisions, gotchas, and cross-project rules.
